...
final class DataAdapter {
	...
	public function __construct($config = array()) {
		...
		switch ($config ["dbdriver"]) {
			case 'mysql' :
				$dsn = $config ["dbdriver"] . ":host=" . $config ["dbserver"] .
						 ";dbname=" . $config ["dbname"];
				break;
			case 'firebird' :
				$dsn = $config ["dbdriver"] . ":dbname=" . $config ["dbserver"] .
						 ":" . $config ["dbname"];
				break;
			case 'oci' :
				$dsn = $config ["dbdriver"] . ":dbname=//" . $config ["dbserver"] . (empty ( $config ["dbport"] ) ? "" : ":" .
	...
	public function execute(\CSI\Data\Sql $sql, $withTransaction = true) {
		...
		try {
			if ($withTransaction) $this->pdo->beginTransaction ();
			if ($sql->isBatch ()) {
				$params = $sql->params ();
			...
			if ($withTransaction) $this->pdo->commit ();
		} catch ( \Exception $e ) {
			if ($withTransaction) $this->pdo->rollBack ();
			throw $e;
		}
		return $this;
	}
